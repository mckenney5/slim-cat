...
#define NAME "cat"   //Program name, can be changed so a user can keep their bloated cat
...
int main(int argc, char *argv[]){
	...
	for(i=1; i < argc; i++){
		if(argv[i][0] == '-' && argv[i][1] != '\0'){ //if its a command line arg
			...
			else if(argv[i][1] == 'u') no_buf = 1; //required by POSIX, turns off buffered input
			else { fprintf(stderr, "%s: unknown command line arg '%s'\n", NAME, argv[i]); return 2;}
		} else break;
	}
	if(argv[i] == NULL) cat(stdin); //if no args
	else for(i=i; i < argc; i++) get_file(argv[i]); //step through each arg as if it's a file
	return 0;
}
